...
import at.bestsolution.efxclipse.runtime.panels.fx.FXTabPane.TabClosingPolicy;
...
public class FXTabPaneSkin extends SkinBase<FXTabPane, FXTabPaneBehavior> {
    ...
    private FXTab selectedTab;
    private FXTab previousSelectedTab;
    private boolean isSelectingTab;

    public FXTabPaneSkin(FXTabPane tabPane) {
        super(tabPane, new FXTabPaneBehavior(tabPane));
        ...
        for (FXTab tab : getSkinnable().getTabs()) {
            addTabContent(tab);
        }
    ...
    private Map<FXTab, Timeline> closedTab = new HashMap();

    private void initializeTabListener() {
        getSkinnable().getTabs().addListener(new ListChangeListener<FXTab>() {
            @Override public void onChanged(final Change<? extends FXTab> c) {      
                while (c.next()) {
                    for (final FXTab tab : c.getRemoved()) {
                    ...
                    int i = 0;
                    for (final FXTab tab : c.getAddedSubList()) {
                        ...
                        if (closedTabTimeline != null) {
                            ...
                            Iterator<FXTab> keys = closedTab.keySet().iterator();
                            while (keys.hasNext()) {
                                FXTab key = keys.next();
                                if (tab.equals(key)) {
                                    removeTab(key);
    ...
    private void addTabContent(FXTab tab) {
    ...
    private void removeTabContent(FXTab tab) {
        for (TabContentRegion contentRegion : tabContentRegions) {
    ...
    private void removeTab(FXTab tab) {
        final TabHeaderSkin tabRegion = tabHeaderArea.getTabHeaderSkin(tab);
    ...
    private boolean isFloatingStyleClass() {
        return getSkinnable().getStyleClass().contains(FXTabPane.STYLE_CLASS_FLOATING);
    }
    ...
    @Override protected void layoutChildren() {
        FXTabPane tabPane = getSkinnable();
    ...
    class TabHeaderArea extends StackPane {
        ...
        public TabHeaderArea() {
            ...
            setManaged(false);
            final FXTabPane tabPane = getSkinnable();
        ...
        private void addTab(FXTab tab, int addToIndex, boolean visible) {
        ...
        private TabHeaderSkin getTabHeaderSkin(FXTab tab) {
    ...
    class TabHeaderSkin extends StackPane {
        ...
        public TabHeaderSkin(final FXTab tab) {
...
//                            getChildren().addAll(tooltip);
            ...
            getProperties().put(FXTab.class, tab);
        ...
        private void removeListeners(FXTab tab) {
   ...
   /**************************************************************************
    ...
    class TabContentRegion extends StackPane implements TraverseListener {
        ...
        public TabContentRegion(FXTab tab) {
    ...
    class TabControlButtons extends StackPane {
        ...
        public TabControlButtons() {            
            ...
            tabPane.getTabs().addListener(new ListChangeListener<FXTab>() {
...
//                popup.setManaged(false);
            ...
            for (final FXTab tab : getSkinnable().getTabs()) {
    ...
    class TabMenuItem extends RadioMenuItem {
        FXTab tab;
        public TabMenuItem(final FXTab tab) {
            super(tab.getText(), FXTabPaneSkin.clone(tab.getGraphic()));                        
            this.tab = tab;
